...
$sqlstr ="select p_code, p_name  from products WHERE category = '$keyword' ORDER BY p_indexs desc";
...
top.<?=$targi?>.options[0].text = "상품명";
<?
while($list = mysql_fetch_array($sqlqry)):
?>
        top.<?=$targi?>.options[<?=$i?>].value = "<?=$list[p_code]?>";
		top.<?=$targi?>.options[<?=$i?>].text = "<?=$list[p_name]?>";
<?
		if($targv==$list[p_code])
?>
		top.<?=$targi?>.options[<?=$i?>].selected = true;
<?
$i++;
endwhile;
}else if($cat == "2"){
